...
resource "yandex_compute_disk" "iscsi-server" {
  count = 1
...
resource "yandex_compute_instance" "nodes" {
  ...
  depends_on  = [yandex_compute_instance.iscsi-server]
...
resource "yandex_compute_instance" "iscsi-server" {
  ...
  platform_id = "standard-v1"
  for_each = {
    for index, vm in var.iscsi-server :
    vm.vm_name => vm
  }
  ...
  dynamic "secondary_disk" {
    for_each = yandex_compute_disk.iscsi-server.*.id
    content {
